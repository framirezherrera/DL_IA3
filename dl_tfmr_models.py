...
# Descripción: Clases de modelos basados en arquitecturas transformers.
...
# La clase DoubleTaskSeq2SeqTransformer es un modelo de Transformer diseñado para realizar dos tareas de traducción de secuencia a secuencia simultáneamente. 
# Implementado por el alumno.
class DoubleTaskSeq2SeqTransformer(nn.Module):

    """
    Esta función se llama al instanciar un objeto de la clase DoubleTaskSeq2SeqTransformer.
    Parámetros de entrada:
        ...
        nhead: Número de cabezas de atención en las capas de atención.
        src_vocab_size: Tamaño del vocabulario de la secuencia fuente.
        tgt_a_vocab_size: Tamaño del vocabulario de la secuencia objetivo A.
        tgt_b_vocab_size: Tamaño del vocabulario de la secuencia objetivo B.
        ff_dim: Dimensión de la capa de feedforward.
        dropout: Tasa de dropout para las capas de dropout.
        weight_sharing: Booleano que indica si se va a compartir los pesos de los generadores y los embeddings de salida.
    """
    def __init__(self, name, num_encoder_layers, num_decoder_layers, emb_size, nhead, src_vocab_size, tgt_a_vocab_size, tgt_b_vocab_size, ff_dim = 512, dropout: float = 0.1, weight_sharing = False):
        super(DoubleTaskSeq2SeqTransformer, self).__init__()
        self.name = name
        # Un modelo de Transformer personalizado con dos decodificadores.
        self.transformer = CustomDoubleDecoderTransformer(d_model=emb_size, head_num=nhead, num_encoder_layers=num_encoder_layers, num_decoder_layers=num_decoder_layers, ff_dim=ff_dim, dropout=dropout)
        # Una capa lineal para generar salidas para la tarea A.
        self.generator_a = nn.Linear(emb_size, tgt_a_vocab_size)
        # Una capa lineal para generar salidas para la tarea B.
        self.generator_b = nn.Linear(emb_size, tgt_b_vocab_size)
        # Un módulo de embedding para tokens de la secuencia fuente.
        self.src_tok_emb = TokenEmbedding(src_vocab_size, emb_size)
        # Un módulo de embedding para tokens de la secuencia objetivo A.
        self.tgt_a_tok_emb = TokenEmbedding(tgt_a_vocab_size, emb_size)
        # Un módulo de embedding para tokens de la secuencia objetivo B.
        self.tgt_b_tok_emb = TokenEmbedding(tgt_b_vocab_size, emb_size)
        # Un módulo de codificación posicional para agregar información de posición a los vectores de embedding.
        self.positional_encoding = PositionalEncoding(emb_size, dropout=dropout)
        # Escala para ajustar la salida de los logits.
        self.x_logit_scale = 1.0
        # Estado que indica si se comparten los pesos de los generadores y los embeddings de salida.
        # Improvement proposed by https://arxiv.org/abs/1706.03762 and implemented 
        # by Felipe Ramírez Herrera.
        self.weight_sharing = weight_sharing       
        if (self.weight_sharing):
            self.tgt_a_tok_emb.shared_weights_with(self.generator_a.weight)
            self.tgt_b_tok_emb.shared_weights_with(self.generator_b.weight)
            self.x_logit_scale = 1.0 / math.sqrt(emb_size)

            """
        Define cómo se calcula la salida del modelo durante el entrenamiento.
        Parámetros de entrada:
            src: Secuencia de entrada.
            tgt_a: Secuencia objetivo A.
            tgt_b: Secuencia objetivo B.
            src_mask: Máscara para ocultar tokens en la secuencia fuente.
            tgt_a_mask: Máscara para ocultar tokens futuros en la secuencia objetivo A.
            tgt_b_mask: Máscara para ocultar tokens futuros en la secuencia objetivo B.
            src_padding_mask: Máscara para ocultar elementos de relleno en la secuencia fuente.
            tgt_a_padding_mask: Máscara para ocultar elementos de relleno en la secuencia objetivo A.
            tgt_b_padding_mask: Máscara para ocultar elementos de relleno en la secuencia objetivo B.
            memory_key_padding_mask: Máscara para ocultar elementos de relleno en la memoria.
        """
        
    def forward(self, src, tgt_a, tgt_b, src_mask, tgt_a_mask, tgt_b_mask, src_padding_mask, tgt_a_padding_mask, tgt_b_padding_mask, memory_key_padding_mask):
        # Convierte las secuencias de entrada y objetivo en vectores de embedding, agregando información de posición.
        src_emb = self.positional_encoding(self.src_tok_emb(src))
        
        tgt_a_emb = self.positional_encoding(self.tgt_a_tok_emb(tgt_a))
        tgt_b_emb = self.positional_encoding(self.tgt_b_tok_emb(tgt_b))

        # Convierte las secuencias de entrada y objetivo en vectores de embedding, agregando información de posición.
        output_a, output_b = self.transformer(src_emb, tgt_a_emb, tgt_b_emb, src_mask, tgt_a_mask , tgt_b_mask, None, src_padding_mask, tgt_a_padding_mask, tgt_b_padding_mask, memory_key_padding_mask)     
        
        # Aplica una capa lineal para generar las salidas finales.
        logits_for_a = self.generator_a(output_a)
        logits_for_b = self.generator_b(output_b)

        # Escalamiento
        if (self.weight_sharing):
            return logits_for_a, logits_for_b
        else:
            return logits_for_a * self.x_logit_scale, logits_for_b * self.x_logit_scale

    # Convierte la secuencia de entrada en vectores de embedding y calcula la representación codificada utilizando el codificador del modelo Transformer.
    def encode(self, src, src_mask):
        return self.transformer.encoder(self.positional_encoding(self.src_tok_emb(src)), src_mask)

    # Permiten realizar únicamente la decodificación utilizando la memoria codificada y una secuencia de entrada en cada decodificador correspondiente.
    def decode_a(self, tgt, memory, tgt_mask, tgt_key_padding_mask, memory_key_padding_mask):
        return self.transformer.decoder_a(self.positional_encoding(self.tgt_a_tok_emb(tgt)), memory, tgt_mask, None, tgt_key_padding_mask, memory_key_padding_mask) 

    # Permiten realizar únicamente la decodificación utilizando la memoria codificada y una secuencia de entrada en cada decodificador correspondiente.
    def decode_b(self, tgt, memory, tgt_mask, tgt_key_padding_mask, memory_key_padding_mask):
        return self.transformer.decoder_b(self.positional_encoding(self.tgt_b_tok_emb(tgt)), memory, tgt_mask, None, tgt_key_padding_mask, memory_key_padding_mask)  
